...
if __name__ == '__main__':
    ...
    all_train_losses_qgen = []
    all_train_losses_guesser = []
    all_train_losses_oracle = []

    all_val_losses_qgen = []
    all_val_losses_guesser = []
    all_val_losses_oracle = []

    exp_config['logging'] = False
    ...
    for epoch in range(optimizer_args['no_epochs']):
        ...
        train_oracle_loss = float_tensor()
        val_oracle_loss = float_tensor()
        ...
        training_oracle_accuracy = list()
        validation_oracle_accuracy = list()
        ...
        for split, dataset in zip(exp_config['splits'], [dataset_train, dataset_val]):
            ...
            for i_batch, sample in tqdm.tqdm(enumerate(dataloader), total=len(dataloader), ncols=50):
                ...
                oracle_loss = to_var(torch.zeros(1))

                decider_accuracy = 0
                ask_accuracy = 0
                guess_accuracy = torch.Tensor([0])
                guesser_accuracy = torch.Tensor([0])
                oracle_accuracy = torch.Tensor([0])
                ...
                for idx, mask in enumerate(masks):
                    ...
                    if idx == 0:
                        ...
                        if epoch % modulo_oracle == 0:
                        # if True:
                            decider_out, oracle_out = model(
                                oracle_question=sample['oracle_question'][mask],
                                oracle_length=sample['oracle_length'][mask],
                                oracle_prev_hist=sample['oracle_prev_hist'][mask],
                                oracle_prev_hist_len=sample['oracle_prev_hist_len'][mask],
                                oracle_spatial=sample['oracle_spatial'][mask],
                                oracle_obj_cat=sample['oracle_obj_cat'][mask],
                                visual_features=avg_img_features[mask],
                                mask_select=2
                            )

                            oracle_loss += oracle_loss_function(oracle_out, sample['oracle_answer'][mask]).unsqueeze(0)
                            oracle_accuracy = calculate_accuracy_oracle(oracle_out, sample['oracle_answer'][mask])

                            decider_loss += ensemble_args['decider']['ask_weight'] * decider_cross_entropy(
                                decider_out.squeeze(1), sample['decider_tgt'][mask])

                    elif idx == 1:
                        if epoch%modulo_guesser != 0:
                            continue
                        else:
                            mask = mask.reshape(-1)
                ...
                if epoch%modulo_oracle != 0 and epoch%modulo_guesser != 0:
                    loss = word_logits_loss
                elif epoch%modulo_oracle == 0 and epoch%modulo_guesser != 0:
                        loss = word_logits_loss + oracle_loss
                elif epoch%modulo_oracle != 0 and epoch%modulo_guesser == 0:
                    loss = guesser_loss + word_logits_loss
                else:
                    loss = guesser_loss + word_logits_loss + oracle_loss
                ...
                if split == 'train':
                    ...
                    if epoch%modulo_oracle != 0 and epoch%modulo_guesser != 0:
                        ...
                        training_oracle_accuracy.append(oracle_accuracy)
                        training_ask_accuracy.append(ask_accuracy)
                        training_guess_accuracy.append(guess_accuracy)
                        train_decision_loss = torch.cat([train_decision_loss, decider_loss.data / batch_size])
                        train_qgen_loss = torch.cat([train_qgen_loss, word_logits_loss.data])
                        train_oracle_loss = torch.cat([train_oracle_loss, oracle_loss.data])
                    elif epoch%modulo_oracle != 0 and epoch%modulo_guesser == 0:
                    ...
                    if exp_config['logging']:
                        visualise.iteration_update(
                            ...
                            oracle_loss=oracle_loss.data[0],
                    ...
                    if epoch%modulo_oracle != 0 and epoch%modulo_guesser != 0:
                        ...
                        validation_oracle_accuracy.append(oracle_accuracy)
                        validation_ask_accuracy.append(ask_accuracy)
                        validation_guess_accuracy.append(guess_accuracy)
                        val_decision_loss = torch.cat([val_decision_loss, decider_loss.data / batch_size])
                        val_qgen_loss = torch.cat([val_qgen_loss, word_logits_loss.data])
                        val_oracle_loss = torch.cat([val_oracle_loss, oracle_loss.data])
                    elif epoch % modulo_oracle != 0 and epoch % modulo_guesser == 0:
                    ...
                    if exp_config['logging']:
                        visualise.iteration_update(
                            ...
                            qgen_loss=word_logits_loss.data[0],
                            guesser_loss=guesser_loss.data[0],
                            oracle_loss=guesser_loss.data[0],
                            decider_loss=decider_loss.data[0]/batch_size,
                            ask_accuracy=ask_accuracy,
                            guess_accuracy=guess_accuracy,
                            guesser_accuracy=guesser_accuracy,
        ...
        if exp_config['save_models'] and not args.breaking:
        ...
        if epoch%modulo_oracle != 0 and epoch%modulo_guesser != 0:
            ...
            print("Epoch %03d, Time taken %.3f, Total Training Loss %.4f, Total Validation Loss %.4f"%(epoch, time()-start, torch.mean(train_total_loss), torch.mean(val_total_loss)))
            print("Training Loss:: QGen %.3f, Decider %.3f, Oracle %.3f"%(torch.mean(train_qgen_loss), torch.mean(train_decision_loss), torch.mean(train_oracle_loss)))
            print("Validation Loss:: QGen %.3f, Decider %.3f, Oracle %.3f"%(torch.mean(val_qgen_loss), torch.mean(val_decision_loss), torch.mean(val_oracle_loss)))
            print("Training Accuracy:: Ask %.3f, Guess  %.3f, Oracle %.3f"%(np.mean(training_ask_accuracy), np.mean(training_guess_accuracy), np.mean(training_oracle_accuracy)))
            print("Validation Accuracy:: Ask %.3f, Guess  %.3f, Oracle %.3f"%(np.mean(validation_ask_accuracy), np.mean(validation_guess_accuracy), np.mean(validation_oracle_accuracy)))
        elif epoch % modulo_oracle != 0 and epoch % modulo_guesser == 0:
            print("Epoch %03d, Time taken %.3f, Total Training Loss %.4f, Total Validation Loss %.4f"%(epoch, time()-start, torch.mean(train_total_loss), torch.mean(val_total_loss)))
            ...
            print("Epoch %03d, Time taken %.3f, Total Training Loss %.4f, Total Validation Loss %.4f"%(epoch, time()-start, torch.mean(train_total_loss), torch.mean(val_total_loss)))
            print("Training Loss:: QGen %.3f, Decider %.3f, Guesser %.3f, Oracle %.3f"%(torch.mean(train_qgen_loss), torch.mean(train_decision_loss), torch.mean(train_guesser_loss), torch.mean(train_oracle_loss)))
            print("Validation Loss:: QGen %.3f, Decider %.3f, Guesser %.3f, Oracle %.3f"%(torch.mean(val_qgen_loss), torch.mean(val_decision_loss), torch.mean(val_guesser_loss), torch.mean(val_oracle_loss)))
            print("Training Accuracy:: Ask %.3f, Guess  %.3f, Guesser %.3f, Oracle %.3f"%(np.mean(training_ask_accuracy), np.mean(training_guess_accuracy), np.mean(training_guesser_accuracy), np.mean(training_oracle_accuracy)))
            print("Validation Accuracy:: Ask %.3f, Guess  %.3f, Guesser %.3f, Oracle %.3f"%(np.mean(validation_ask_accuracy), np.mean(validation_guess_accuracy), np.mean(validation_guesser_accuracy), np.mean(validation_oracle_accuracy)))
        ...
        if exp_config['logging']:
            visualise.epoch_update(
                ...
                train_oracle_loss=0 if (epoch % args.modulo != 0) else torch.mean(train_oracle_loss),
                ...
                train_oracle_accuracy=0 if (epoch % args.modulo != 0) else np.mean(training_oracle_accuracy),
                ...
                valid_oracle_loss=0 if (epoch % args.modulo != 0) else torch.mean(val_oracle_loss),
                ...
                valid_oracle_accuracy=0 if (epoch % args.modulo != 0) else np.mean(validation_oracle_accuracy),
                epoch=epoch,
                modulo=args.modulo
            )
